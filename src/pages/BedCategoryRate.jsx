...
const BedCategoryRate = () => {
  ...
  const updateRate = (idx, value) => {
    ...
    setCategories((c) => c.map((cat, i) => (i === idx ? { ...cat, rate } : cat)));
  };
    ...
    <div className="container-fluid" style={{ maxWidth: 700 }}>
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body">
            ...
            <table className="table align-middle">
              <thead>
                <tr>
                  ...
                  <th className="text-muted small text-uppercase">Daily Rate</th>
                </tr>
              </thead>
              <tbody>
                {categories.map((c, idx) => (
                  <tr key={c.name}>
                    <td><span className="badge bg-secondary">{c.name}</span></td>
          ...
          <div className="text-muted small">Rates are stored locally for demo.</div>
        </div>
      </div>
    </div>
  );
